...
class RNN(object):
    def __init__(self,
        ...
        with tf.variable_scope('logits'):
            ...
            fc = tf.nn.tanh(tf.einsum("aij,jk->aik", H, Ws1))
            A_t = tf.nn.softmax(tf.einsum("aij,jk->aik", fc, Ws2), axis=1)
            M_t = tf.matmul(tf.transpose(H, perm=[0, 2, 1]), A_t)  # shape: [batch, param_r, 2*num_units]
        ...
        self.penalized_term = tf.reduce_mean(tf.square(tf.norm(tf.matmul(A, A_t) - identity, ord='euclidean', axis=[1, 2])))

        #with tf.name_scope('loss'):
        # ???
        #self.loss = tf.reduce_mean(tf.reduce_sum(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels, logits=logits)), name='loss') + 0.0001*self.penalized_term
        ...
        self.train_merged_summary = tf.summary.merge([self.loss_summary, self.acc_summary])
    ...
    def train_step(self, session, data):
        ...
        output_feed = [self.train_merged_summary, self.loss, self.accuracy, self.gradient_norm, self.update]
