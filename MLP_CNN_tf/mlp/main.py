...
# 重定向即可输出的result， 然后使用draw.py解析，绘制图像
...
tf.app.flags.DEFINE_string("gpu", "0", "gpu config")
...
def train_epoch(model, sess, X, y, writer):  # Training Process
    global train_step
    loss, acc = 0.0, 0.0
    st, ed, times = 0, FLAGS.batch_size, 0
    while st < len(X) and ed <= len(X):
        X_batch, y_batch = X[st:ed], y[st:ed]
        # feed的作用是给使用placeholder创建出来的tensor赋值
        feed = {model.x_: X_batch, model.y_: y_batch} # 交互的关键在此，设置模型的输入，由模型给出输出
        loss_, acc_, _, summary = sess.run(fetches=[model.loss, model.acc, model.train_op, model.train_summary_op], feed_dict=feed)
...
def valid_epoch(model, sess, X, y, writer):  # Valid Process
    global valid_step
    loss, acc = 0.0, 0.0
    st, ed, times = 0, FLAGS.batch_size, 0
    while st < len(X) and ed <= len(X):
        ...
        loss_, acc_, summary = sess.run(fetches=[model.loss_val, model.acc_val, model.valid_summary_op], feed_dict=feed)
        writer.add_summary(summary, valid_step)
        loss += loss_
        acc += acc_
        st, ed = ed, ed+FLAGS.batch_size
        times += 1
...
with tf.Session(config=config) as sess:
    ...
    if FLAGS.is_train:
        with tf.device("/gpu:0"):  # for gpu:0
        ...
        writer = tf.summary.FileWriter(tensorboard_dir + "test", graph=sess.graph)
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            train_acc, train_loss = train_epoch(mlp_model, sess, X_train, y_train, writer)  # Complete the training process
            
            X_train, y_train = shuffle(X_train, y_train, 1)
            val_acc, val_loss = valid_epoch(mlp_model, sess, X_val, y_val, writer)  # Complete the valid process

            if val_acc >= best_val_acc:  # when valid_accuracy > best_valid_accuracy, save the model
                best_val_acc = val_acc
                best_epoch = epoch + 1
                test_acc, test_loss = valid_epoch(mlp_model, sess, X_test, y_test, writer)  # Complete the test process
