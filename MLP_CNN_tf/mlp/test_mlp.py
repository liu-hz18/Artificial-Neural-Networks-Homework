...
with tf.Session() as sess:
    ...
    with tf.variable_scope('y1'):
        BN_true_ans_ = tf.layers.batch_normalization(x, 1, training=True)
        BN_false_ans_ = tf.layers.batch_normalization(x, 1, training=False)
    ...
    inputs = np.random.uniform(low=-0.1, high=1.1, size=(128, 256))
